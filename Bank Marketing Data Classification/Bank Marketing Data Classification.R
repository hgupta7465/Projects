...
is.null(dataset)
#the is.null() function returned us FALSE value this means that there no 
#NULL values in our dataset.

#Now, we will simply visualize some of the columns in the dataset in 
#order to get a general information about the clients. 
#
...
#Encoding categorical data for contact column in the dataset.
...
#The Random Forest algorithm is chosen for the variety of classes, 
#numeric/categorical, within the Bank Marketing dataset. 
#Random Forest is also probability-based, thereby compensating for the 
#distance-based scheme of Support Vector Machines.
...
colnames(grid_set) = c('Age', 'Call Duration')
y_grid = predict(classifier, grid_set)
plot(set[, -21], main = 'Random Forest Classification (Test set)',
     xlab = 'Age', ylab = 'Call Duration',
     xlim = range(X1), ylim = range(X2))
contour(X1, X2, matrix(as.numeric(y_grid), length(X1), length(X2)), add = TRUE)
points(grid_set, pch = '.', col = ifelse(y_grid == 1, 'springgreen3', 'tomato'))
